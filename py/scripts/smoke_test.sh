...
#
# Local build + install smoke test: prove `tenant-emit --version` resolves through
...
#   4. run the installed `tenant-emit --version` and assert it works + round-trips
#
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/tenant-emit-py-smoke.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT INT TERM

say "building host binary (cargo build --release --bin tenant-emit)..."
cargo build --release --bin tenant-emit --manifest-path "${REPO_ROOT}/Cargo.toml" >/dev/null
HOST_BIN="${REPO_ROOT}/target/release/tenant-emit"
...
WHEEL="$(ls "${WHEEL_OUT}"/tenant_emit-*.whl)"
...
if have uv; then
  ...
  RUN="${UV_TOOL_BIN_DIR}/tenant-emit"
else
  say "uv not found; installing into a venv with pip..."
  "$PYBIN" -m venv "${WORK}/venv"
  "${WORK}/venv/bin/pip" install --quiet "$WHEEL"
  RUN="${WORK}/venv/bin/tenant-emit"
fi
[ -x "$RUN" ] || die "installed launcher not found/executable at ${RUN}"

say "running: tenant-emit --version"
OUT="$("$RUN" --version)"
say "  -> ${OUT}"
echo "$OUT" | grep -q "$VERSION" || die "version output '${OUT}' did not contain ${VERSION}"
"$RUN" --help >/dev/null || die "tenant-emit --help failed through the installed binary"

say "OK: wheel installs and the bundled binary runs as the tenant-emit command"
