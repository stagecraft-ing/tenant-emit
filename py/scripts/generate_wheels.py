...
Two input modes (mirroring the npm generator):
  --archives <dir>   extract binaries from tenant-emit-<tag>-<triple>.{tar.gz,zip}
...
Usage:
  ...
  python scripts/generate_wheels.py --target linux-x64 \
      --binary ../target/release/tenant-emit
"""
...
from tenant_emit import platform_map as pm  # noqa: E402  (after sys.path setup)

REPO_ROOT = PY_DIR.parent
GENERATOR = "tenant-emit-generate-wheels 1.0"
...
def _extract_from_archive(target: str, archives_dir: Path, tag: str, bin_file: str) -> Path:
    ...
    members are stored "./"-prefixed (./tenant-emit). A strict tar (GNU tar) will
    not match a bare `tenant-emit`; extracting everything lands the binary at
    <tmp>/<bin_file> on every tar/zip flavor. (The npm generator learned this the
    hard way; see spec 002.)
    """
    rec = pm.TARGETS[target]
    is_win = rec["windows"]
    ext = "zip" if is_win else "tar.gz"
    archive = archives_dir / f"tenant-emit-{tag}-{rec['triple']}.{ext}"
    if not archive.exists():
        die(f"archive not found for {target}: {archive}")
    tmp = Path(tempfile.mkdtemp(prefix="tenant-emit-extract-"))
    if is_win:
...
def _metadata(version: str, requires_python: str) -> bytes:
    body = (
        ...
        f"Name: {pm.DIST_NAME}\n"
        f"Version: {version}\n"
        "Summary: The tenant-emit governance-certificate toolkit: produce a "
        "factory's run-side governance certificate (build-certificate) with no "
        "trust handed to the producer beyond the operator key it signs with. "
        "Ships the prebuilt binary; no Rust toolchain required.\n"
        "License-Expression: Apache-2.0\n"
        "License-File: LICENSE\n"
        "Project-URL: Homepage, https://github.com/statecrafting/tenant-emit\n"
        "Project-URL: Repository, https://github.com/statecrafting/tenant-emit\n"
        "Project-URL: Issues, https://github.com/statecrafting/tenant-emit/issues\n"
        ...
        "directly. See https://github.com/statecrafting/tenant-emit#readme.\n"
    )
...
def verify_version_lock(version: str) -> None:
    ...
    if current != version:
        die(
            "version lock mismatch (spec 002 §3.5):\n"
