# tenant-emit (Python / uvx)

The `tenant-emit` governance-certificate toolkit for Python users: produce a
factory's run-side governance certificate (`build-certificate`) with no trust
handed to the producer beyond the operator key it signs with. Ships the prebuilt
binary; **no Rust toolchain required**.

```sh
uvx tenant-emit build-certificate     # run the CLI with no install
uv tool install tenant-emit           # or install it as a persistent tool
pip install tenant-emit               # or into a project/venv
```
...
- **five platform wheels**, one per supported target, each carrying the prebuilt
  `tenant-emit` binary in the wheel's scripts directory. pip/uv select the one
...
- **one sdist**, the unsupported-host fallback. It builds only when no wheel
  matches (musl/Alpine, win-arm64, 32-bit), and its `tenant-emit` command prints a
  clear message pointing at `cargo install tenant-emit-cli`.

This mirrors the npm shim (spec 002): npm uses `os`/`cpu`-gated
...
`cargo install tenant-emit-cli` or a glibc-based image.
