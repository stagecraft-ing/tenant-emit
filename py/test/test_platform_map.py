...
from tenant_emit import platform_map as pm


class PlatformMapTest(unittest.TestCase):
    def test_maps_every_supported_target(self):
        cases = [
            ("darwin", "arm64", "aarch64-apple-darwin", "macosx_11_0_arm64", "tenant-emit"),
            ("darwin", "x64", "x86_64-apple-darwin", "macosx_10_12_x86_64", "tenant-emit"),
            ("linux", "x64", "x86_64-unknown-linux-gnu", "manylinux_2_17_x86_64", "tenant-emit"),
            ("linux", "arm64", "aarch64-unknown-linux-gnu", "manylinux_2_17_aarch64", "tenant-emit"),
            ("win32", "x64", "x86_64-pc-windows-msvc", "win_amd64", "tenant-emit.exe"),
        ]
    ...
    def test_refuses_unsupported_targets_with_source_hint(self):
        for os_name, cpu in [("win32", "arm64"), ("linux", "ia32"),
            ...
            self.assertIn("cargo install tenant-emit-cli", msg)
    ...
    def test_binary_name(self):
        self.assertEqual(pm.binary_name(False), "tenant-emit")
        self.assertEqual(pm.binary_name(True), "tenant-emit.exe")
