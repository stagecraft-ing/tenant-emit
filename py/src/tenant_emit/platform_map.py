...
npm/lib/platform.js's SUPPORTED map and the platform table in spec 002: the five
...
# normalizes "tenant-emit" <-> "tenant_emit"; the .data/scripts and .dist-info
# directories inside a wheel use the underscore form.
DIST_NAME = "tenant-emit"
DIST_STEM = "tenant_emit"
...
def binary_name(windows: bool) -> str:
    ...
    return "tenant-emit.exe" if windows else "tenant-emit"
...
def unsupported_message(os_name: str, cpu: str, reason: str | None = None) -> str:
    ...
    lines = [
        f"tenant-emit: no prebuilt binary for {host}.",
    ...
    if reason == "musl":
        lines.append(
            "(Alpine/musl: use a glibc-based image, or cargo install tenant-emit-cli.)"
        )
    lines.append(
