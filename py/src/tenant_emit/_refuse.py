"""Spec: specs/002-distribution/spec.md (unsupported hosts fail clearly).

The `tenant-emit` console entry point shipped by the sdist. It is reached only on
a host with no prebuilt wheel (musl/Alpine, win-arm64, 32-bit, ...), because on a
supported host the platform wheel's bundled binary IS the `tenant-emit` command
...
if __name__ == "__main__":  # `python -m tenant_emit._refuse`
