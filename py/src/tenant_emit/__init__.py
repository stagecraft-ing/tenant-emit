"""tenant-emit: Python/uvx distribution.

On a supported host you never import this package: pip/uv select a platform
wheel whose only payload is the prebuilt `tenant-emit` binary in the wheel's
scripts directory, so `tenant-emit`/`uvx tenant-emit` runs the native binary with
...
`tenant-emit` entry point prints a clear refusal (see _refuse.py). Spec:
specs/002-distribution/spec.md.
"""
...
try:  # populated from installed dist metadata; no hardcoded second source of truth
    ...
    __version__ = _version("tenant-emit")
