...
tenant-emit is an emit-only CLI (it produces a factory's run-side governance certificate, signing with an operator key) and is never a verifier. A three-crate Cargo workspace plus an npm wrapper.

| Surface | Path | Build / verify |
|---------|------|----------------|
| Spec corpus | `specs/NNN-slug/spec.md` | `npx --no-install spec-spine compile`, then `... lint` |
| Shared DTOs | `crates/tenant-emit-types/` | `cargo check` / `cargo build` |
| Emit engine | `crates/tenant-emit-core/` | `cargo check`, `cargo test -p tenant-emit-core` |
| CLI crate | `crates/tenant-emit-cli/` | `cargo build --release -p tenant-emit-cli` |
...
**Emit-only boundary (do not violate):** never add a verifier path (signature re-check, artifact re-derivation, corpus-binding adjudication), a verifier dependency, or any network surface. The corpus binding is read, never recomputed (the emit-side attestation-emit / corpus-recompute surface is banned workspace-wide via clippy.toml and deny.toml). `unsafe_code = "forbid"` is workspace-wide. If a plan would cross this boundary, stop and surface it rather than implementing it.
...
After each change:
- **Rust**: `cargo check` (fast) or `cargo build` (full); run `cargo test --workspace` (or `cargo test -p tenant-emit-core <name>`) when behavior changed
...
- **DO NOT:** Add `unsafe`, a verifier path, or a network surface, or recompute the corpus binding; the emit-only boundary is structural
