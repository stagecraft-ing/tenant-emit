...
argument-hint: "[scope] - e.g. \"branch\", \"working tree\", \"crates/tenant-emit-core\""
---
...
This repo is an emit-only Rust workspace plus an npm and a PyPI
...
- The emit-only boundary: no verifier path (no signature re-check, no
  artifact re-derivation, no corpus-binding adjudication), no network/HTTP
  client. The corpus binding is read, never recomputed (the emit-side
  attestation-emit / corpus-recompute surface is banned workspace-wide via
  clippy.toml and deny.toml). `unsafe_code` is forbidden workspace-wide. A
  change that crosses this boundary is a headline finding regardless of
  correctness.
- Determinism hazards: unsorted map/set iteration, locale- or
  platform-dependent behavior, unstable ordering in emitted JSON or
  hashes. The certificate self-hash and signature are promised
...
- Error-path correctness: does the verb return the right exit-code class
  (0 certificate emitted, 2 usage or I/O error, including a
