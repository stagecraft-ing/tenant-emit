...
test('maps every supported triple to its scoped package and binary name', () => {
  const cases = [
    ['darwin', 'arm64', '@tenant-emit/cli-darwin-arm64', 'tenant-emit'],
    ['darwin', 'x64', '@tenant-emit/cli-darwin-x64', 'tenant-emit'],
    ['linux', 'x64', '@tenant-emit/cli-linux-x64', 'tenant-emit'],
    ['linux', 'arm64', '@tenant-emit/cli-linux-arm64', 'tenant-emit'],
    ['win32', 'x64', '@tenant-emit/cli-win32-x64', 'tenant-emit.exe'],
  ];
  ...
  for (const [os, arch] of [
    ...
    assert.throws(
      ...
      (err) => {
        ...
        assert.match(err.message, /cargo install tenant-emit-cli/);
        return true;
      },
...
test('refuses musl Linux even on an otherwise-supported arch', () => {
  ...
  assert.equal(targetFor('linux', 'x64', { muslLinux: false }).packageName, '@tenant-emit/cli-linux-x64');
});
...
test('resolveBinaryPath joins the resolved package dir with bin/<name>', () => {
  ...
  const resolve = (request) => {
    assert.equal(request, '@tenant-emit/cli-darwin-arm64/package.json');
    return path.join('/fake/node_modules/@tenant-emit/cli-darwin-arm64', 'package.json');
  };
  const got = resolveBinaryPath({ process: fakeProcess, resolve });
  assert.equal(got, path.join('/fake/node_modules/@tenant-emit/cli-darwin-arm64', 'bin', 'tenant-emit'));
});
...
test('resolveBinaryPath reports a missing optional package clearly', () => {
  ...
  assert.throws(
    ...
    (err) => {
      assert.ok(err instanceof UnsupportedPlatformError);
      assert.match(err.message, /@tenant-emit\/cli-darwin-x64/);
