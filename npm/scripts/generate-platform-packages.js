...
// Assemble the per-triple platform packages (@tenant-emit/cli-<os>-<cpu>) from
...
function extractBinary(target, archivesDir, tag, binFile) {
  ...
  const archive = path.join(
    archivesDir,
    `tenant-emit-${tag}-${t.triple}.${isWin ? 'zip' : 'tar.gz'}`,
  );
  if (!fs.existsSync(archive)) {
    die(`archive not found for ${target}: ${archive}`);
  }
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'tenant-emit-extract-'));
  ...
  // (./tenant-emit). GNU tar (the Linux runner) will not match a bare
  // `tenant-emit` against `./tenant-emit` and errors "Not found in archive";
...
function platformPackageJson(target, version) {
  const t = TARGETS[target];
  return {
    name: `@tenant-emit/cli-${target}`,
    version,
    description: `Prebuilt tenant-emit CLI binary for ${target}. Installed automatically by the \`tenant-emit\` package; do not depend on it directly.`,
    license: 'Apache-2.0',
    repository: {
      type: 'git',
      url: 'git+https://github.com/statecrafting/tenant-emit.git',
    },
    os: [t.os],
    cpu: [t.cpu],
    files: ['bin/'],
...
function generateOne(target, version, tag, outRoot, opts) {
  const t = TARGETS[target];
  const binFile = t.os === 'win32' ? 'tenant-emit.exe' : 'tenant-emit';
  ...
  const pkgDir = path.join(outRoot, '@tenant-emit', `cli-${target}`);
  ...
  fs.writeFileSync(
    ...
    `# @tenant-emit/cli-${target}\n\n` +
      `Prebuilt \`tenant-emit\` binary for \`${target}\` (${t.triple}).\n\n` +
      'This package is installed automatically as an optional dependency of the\n' +
      '[`tenant-emit`](https://www.npmjs.com/package/tenant-emit) package. Do not\n' +
...
function lockMainPackage(version) {
  ...
  for (const target of Object.keys(TARGETS)) {
    pkg.optionalDependencies[`@tenant-emit/cli-${target}`] = version;
  }
  writeJson(file, pkg);
...
function verifyMainLock(version) {
  ...
  for (const target of Object.keys(TARGETS)) {
    const name = `@tenant-emit/cli-${target}`;
