#!/usr/bin/env bash
# Local pack + install smoke test: prove `tenant-emit --version` resolves through
...
#   5. run the installed `tenant-emit --version` and assert it works
#
...
case "$TARGET" in
  darwin-arm64|darwin-x64|linux-x64|linux-arm64) BIN="tenant-emit" ;;
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/tenant-emit-smoke.XXXXXX")"
...
PLATFORM_PKG_DIR="${PKG_OUT}/@tenant-emit/cli-${TARGET}"
...
BIN_LINK="${PROJ}/node_modules/.bin/tenant-emit"
[ -e "$BIN_LINK" ] || die "launcher not linked at ${BIN_LINK}"

say "running the installed launcher: tenant-emit --version"
...
"$BIN_LINK" --help >/dev/null || die "tenant-emit --help failed through the launcher"
