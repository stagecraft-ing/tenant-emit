...
// the launcher (bin/tenant-emit.js). No I/O on the mapping path, so it is
...
// in lockstep.
const SUPPORTED = {
  'darwin-arm64': '@tenant-emit/cli-darwin-arm64',
  'darwin-x64': '@tenant-emit/cli-darwin-x64',
  'linux-x64': '@tenant-emit/cli-linux-x64',
  'linux-arm64': '@tenant-emit/cli-linux-arm64',
  'win32-x64': '@tenant-emit/cli-win32-x64',
};
...
function binaryName(platform) {
  return platform === 'win32' ? 'tenant-emit.exe' : 'tenant-emit';
}
...
function unsupportedMessage(platform, arch, reason) {
  ...
  const lines = [
    `tenant-emit: no prebuilt binary for ${host}.`,
  ...
  if (reason === 'musl') {
    lines.push('(Alpine/musl: use a glibc-based image, or cargo install tenant-emit-cli.)');
  }
  return lines.join('\n');
}
...
function missingPackageMessage(packageName) {
  return [
    `tenant-emit: the prebuilt binary package '${packageName}' is not installed.`,
